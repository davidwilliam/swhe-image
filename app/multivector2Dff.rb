module X
  class Multivector2Dff < Multivector
    attr_accessor :e0, :e1, :e2, :e12, :v1, :v2,
                  :w1, :w2, :v, :w, :modulus

    def self.multivector_for(n)
      while true
        data = Array.new(4){X::SWHE.random_number(8)}
        m = new(data)
    ...
    def geometric_product(m2)
      m = self.clone
      m.e0 = (self.e0*m2.e0) + (self.e1*m2.e1) +
             (self.e2*m2.e2) - (self.e12*m2.e12)
      m.e1 = (self.e0*m2.e1) + (self.e1*m2.e0) -
             (self.e2*m2.e12) + (self.e12*m2.e2)
      m.e2 = (self.e0*m2.e2) + (self.e1*m2.e12) +
             (self.e2*m2.e0) - (self.e12*m2.e1)
      m.e12 = (self.e0*m2.e12) + (self.e1*m2.e2) -
             (self.e2*m2.e1) + (self.e12*m2.e0)
      m.mod
    end
    ...
    def clifford_conjugation
      signs = [1,-1,-1,-1]
      m = self.clone
      m.data self.data.map.with_index{|d,i|
        signs[i] == 1 ? d : -d}; m.mod
    end
    ...
    def get_eigenvectors
      ...
      v1_gcd_v2 = (v1_.numerator *
        v2_.denominator).gcd(v2_.numerator * v1_.denominator)
      ...
      w1_gcd_w2 = (w1_.numerator *
        w2_.denominator).gcd(w2_.numerator * w1_.denominator)
    ...
    def pow2(n)
      e_ = self.class.new [
        Rational(1,2) * ((Rational(n,2) + Rational(n,2)) +
        (Rational(n,2) - Rational(n,2))),
        Rational(1,2) * ((Rational(n,2) + Rational(n,2)) -
        (Rational(n,2) - Rational(n,2))),
        Rational(1,2) * ((Rational(n,2) + Rational(n,2)) +
        (Rational(n,2) - Rational(n,2))),
    ...
    def to_m(m)
      if m.rationalize.e0.to_i.even?
        self.scalar_mul(self.rationalize2.e0 **
          ((m.rationalize2.e0 - 1)/2).to_i).gp(self)
      else
        self.scalar_mul(self.rationalize2.e0 **
          ((m.rationalize2.e0 - 1)/2).to_i)
      end
    end
