module X
  class Padic
    ...
    # RETURNS:    hensel code, which enables
    # the generation of finite p-adic digits
    ...
    def decode
      Tools.ctr(p ** r, Padic.digits_to_hensel_code(p, r, digits),
      Math.sqrt(p**r/2))
    end
    ...
    def add b
      ...
      carry = 0
      (0..r-1).each do |i|
        carry, mod = (self.digits[i] +
          b.digits[i] + carry).divmod(self.p)
        result << mod
      end
